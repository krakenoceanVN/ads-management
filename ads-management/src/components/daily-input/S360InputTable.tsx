...
export default function S360InputTable({ date, search = '' }: Props) {
  ...
  const { data: rows = [], isLoading, isError } = useQuery({
    queryKey: ['daily-input', '360', date, search],
    queryFn: () =>
      api.get<ApiResponse<DailyInputRow[]>>('/api/daily-input', { params: { date, ad_type: '360', search: search || undefined } })
        .then((r) => r.data.data ?? []),
  })
  ...
  const mutation = useMutation({
    mutationFn: (records: { ad_site_id: number; amount1?: number; amount2?: number; ratio_override?: number; qty?: number }[]) =>
      api.post('/api/daily-input/batch', { date, ad_type: '360', records }),
    onSuccess: () => {
  ...
  const handleSave = useCallback(() => {
    mutation.mutate(
      Object.entries(drafts).map(([id, d]) => ({
        ad_site_id: +id,
        amount1: (d as Draft360[number]).amount1,
        amount2: (d as Draft360[number]).amount2,
        ratio_override: (d as Draft360[number]).ratio_override,
        qty: (d as Draft360[number]).qty,
      }))
    )
  }, [drafts, mutation])
              ...
              <Table.Summary fixed="bottom">
                <Table.Summary.Row>
                  ...
                  <Table.Summary.Cell index={qtyColumnIndex}>
                    <strong>{rows.reduce((s: number, r: DailyInputRow) => s + (drafts[r.id]?.qty ?? r.existing_record?.qty ?? 0), 0).toLocaleString()}</strong>
                  </Table.Summary.Cell>
