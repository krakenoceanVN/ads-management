...
exports.TAX_RATE = void 0;
exports.normalizePctHal = normalizePctHal;
exports.resolveDownstreamRate = resolveDownstreamRate;
exports.calculateCost = calculateCost;
exports.calculateProfit = calculateProfit;
exports.aggregateDownstreamCost = aggregateDownstreamCost;
const client_1 = require("../../shared/prisma/client");
const revenue_service_1 = require("./revenue.service");
exports.TAX_RATE = 0.06;
 ...
 */
async function resolveDownstreamRate(adSiteId, downstreamId, recordDate) {
    // 1. AdSiteDownstream.customPrice — scoped to this adSiteId + downstreamId
    const customPriceRow = await client_1.prisma.adSiteDownstream.findFirst({
        where: {
            adSiteId,
            downstreamId,
            status: 'active',
    ...
    const dailyRate = await client_1.prisma.dailyDownstreamRate.findFirst({
        where: {
            downstreamId,
            date: recordDate,
        },
    ...
    // 3. DownstreamPeriod.unitPrice — this downstreamId, active on recordDate
    const period = await client_1.prisma.downstreamPeriod.findFirst({
        where: {
            downstreamId,
 ...
 */
function calculateCost(billingMethod, qty, rate, shareRatio, amount1, amount2) {
    const q = toNum(qty);
    const r = toNum(rate);
...
function calculateProfit(revenue, cost) {
    ...
    const tax = grossProfit > 0 ? roundCurrency(grossProfit * exports.TAX_RATE) : 0;
 ...
 */
async function aggregateDownstreamCost(inputs) {
    let totalCost = 0;
    const errors = [];
    for (const di of inputs) {
        ...
        for (const j of downstreams) {
            ...
            try {
                ...
                const cost = calculateCost(billingMethod, toNum(di.qty), rate, shareRatio, toNum(di.amount1), toNum(di.amount2));
                recordCost += cost;
            }
            catch (err) {
