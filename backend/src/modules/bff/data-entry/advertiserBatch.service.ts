    ...
    try {
      ...
      if (existing) {
        ...
        await prisma.dailyInput.update({
          ...
          data: {
            qty: item.qty ?? 0,
            unitPriceSnapshot: item.unitPrice != null && String(item.unitPrice).trim() !== '' ? new Prisma.Decimal(String(item.unitPrice)) : new Prisma.Decimal(String(site.currentUnitPrice ?? 0)),
            ...
            ratioSnapshot: item.ratio != null && String(item.ratio).trim() !== '' ? new Prisma.Decimal(String(item.ratio)) : new Prisma.Decimal(String(site.currentRatio ?? 0)),
