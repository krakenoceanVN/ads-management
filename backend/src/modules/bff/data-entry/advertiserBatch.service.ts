...
function validateAdvertiserBatchItem(item: AdvertiserBatchItem, billingMethod: string): void {
    ...
    try {
      ...
      if (existing) {
        ...
        await prisma.dailyInput.update({
          ...
          data: {
            qty: item.qty != null && String(item.qty).trim() !== '' ? Number(item.qty) : 0,
            unitPriceSnapshot: item.unitPrice != null && String(item.unitPrice).trim() !== '' ? new Prisma.Decimal(String(item.unitPrice)) : new Prisma.Decimal(site.currentUnitPrice != null ? String(site.currentUnitPrice) : '0'),
            amount1: item.amount1 != null && String(item.amount1).trim() !== '' ? new Prisma.Decimal(String(item.amount1)) : undefined,
            amount2: item.amount2 != null && String(item.amount2).trim() !== '' ? new Prisma.Decimal(String(item.amount2)) : undefined,
            ratioSnapshot: item.ratio != null && String(item.ratio).trim() !== '' ? new Prisma.Decimal(String(item.ratio)) : new Prisma.Decimal(site.currentRatio != null ? String(site.currentRatio) : '0'),
