/**
 * BFF Media DataEntry Controller
 * Handles Media-side daily data entry rows (downstream perspective)
 * 
 * Uses shared saveDailyInputBatch() workflow from workflows/dailyInputBatch.workflow.ts
 * 
 * Key differences from Advertiser:
 * - actualReceived is computed as revenue * shareRatio (display only)
 * - shareRatio comes from Downstream.payoutRate
 * - No inheritance - frontend must send explicit values
 */
...
// ============================================================
// GET /api/bff/data-entry/media
// Query: date (YYYY-MM-DD), mediaId (optional), adTypeCode (optional)
// ============================================================
router.get(
    "/",
    requireAuth,
    ...
    async (req: Request, res: Response) => {
        try {
            ...
            console.error("GET /api/bff/data-entry/media error:", err);
...
// ============================================================
// POST /api/bff/data-entry/media/batch
// Body: { date, adTypeCode, records: MediaBatchItem[] }
...
router.post(
    ...
    async (req: Request, res: Response) => {
        try {
            ...
            // Call shared workflow (no userId needed for media-side, pass 0 or get from auth)
            ...
            createOperationLog({
                ...
                targetId: null,
                detail: `Saved media data entry batch: date=${date}, adTypeCode=${adTypeCode}, count=${records.length}`,
            });

            res.json(result);
        } catch (err: any) {
            console.error("POST /api/bff/data-entry/media/batch error:", err);
            res.status(500).json({ success: false, error: "Internal server error" });
        }
    }
);
...
// ============================================================
// POST /api/bff/data-entry/media/confirm-batch
// Body: { ids: number[] }
// ============================================================
router.post(
    "/confirm-batch",
    ...
    async (req: Request, res: Response) => {
        try {
            ...
            createOperationLog({
                ...
                targetType: "DailyInput",
                targetId: null,
                detail: `Confirmed media data entries: count=${result.count}`,
            });

            res.json({ success: true, updated: result.count });
        } catch (err: any) {
            console.error("POST /api/bff/data-entry/media/confirm-batch error:", err);
            res.status(500).json({ success: false, error: "Internal server error" });
        }
    }
);
...
// ============================================================
// PUT /api/bff/data-entry/media/:id/unconfirm
// ============================================================
router.put(
    "/:id/unconfirm",
    requireAuth,
    ...
    async (req: Request, res: Response) => {
        try {
            ...
            createOperationLog({
                ...
                targetType: "DailyInput",
                targetId: String(id),
                detail: `Unconfirmed media data entry id=${id}`,
            });

            res.json({ success: true, message: "Unconfirmed" });
        } catch (err: any) {
            console.error("PUT /api/bff/data-entry/media/:id/unconfirm error:", err);
            res.status(500).json({ success: false, error: "Internal server error" });
        }
    }
);
